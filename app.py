...
def page_search(title):
    # Фильм по названию
    result = my_film.get_by_title(title)
...
def page_range_year(first_year, second_year):
    ...
    result = my_film.get_by_range(first_year, second_year)
    return app.response_class(
        json.dumps(result, ensure_ascii=False, indent=4),
        status=200,
        mimetype="application/json")
...
def page_children_rating():
    # для детей
    rating = "'G'"
    result = my_film.get_by_rating(rating)
    return app.response_class(
        json.dumps(result, ensure_ascii=False, indent=4),
        status=200,
        mimetype="application/json")
...
def page_genre_name(genre):
    # Фильмы по жанру
    result = my_film.get_by_genre(genre)
    return app.response_class(
        json.dumps(result, ensure_ascii=False, indent=4),
        status=200,
        mimetype="application/json")
